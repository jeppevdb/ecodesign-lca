...
# shared “master” component selector
from impact_report_generation.src.tabs.design_changes.component_redesigns.generate_intro import global_component_sel
from impact_report_generation.src.utils.selectors import get_scenarios, get_countries, characterized_impact_selector_with_score, impact_unit_label, local_design_component_selector
...
def build():
    ...
    df_comp = pd.concat([df_base, df_vars], ignore_index=True)
    ...
    def view(global_sel, local_sel, impact):
        ...
        # df_co = df_comp[df_comp["design_variation_component"] == comp]
        df_c = df_comp[(df_comp["component"] == comp)]
        rows = []
        for country in countries:
            ...
            for scen in scenarios:
                ...
                base = df_ct[
                    ...
                    # (df_ct.component == comp) &
                    (df_ct.scenario_setting     == scen)
                ].copy()
                var  = df_ct[
                    (df_ct.design_variation_type != "base_design") &
                    # (df_ct.component == comp) &
                    (df_ct.design_variation_component == comp) &
                    (df_ct.scenario_setting     == scen)
                ].copy()
                ...
                agg_base = (
                    base
                    .groupby(["variation","process_description"])[impact]
                    .sum()
                    .reset_index()
                )
                agg_var = (
                ...
                bars = hv.Bars(
                    agg,
                    kdims=["variation","process_description"],
    ...
    # 6) Wrap it up
    header   = pane.Markdown("## Component-Level Redesign Stacks")
    controls = pn.Row(  
                    global_display,
                    local_comp,
                    impact_sel,
